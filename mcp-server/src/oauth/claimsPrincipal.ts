...
export class ClaimsPrincipal {
    ...
    public enforceRequiredScope() {
        ...
        if (scopes.indexOf(this.configuration.requiredScope) === -1) {
            ...
            const error = new McpServerError(403, 'insufficient_scope', 'The access token does not contain the required scope');
            error.extra = {
                message: this.scope,
            };
            throw error;
    ...
    private getClaim(claims: JWTPayload, name: string, required = true): any {
        ...
        if (!value && required) {
            throw new McpServerError(403, 'insufficient_data', `The access token does not contain the required ${name} claim`);
        }
