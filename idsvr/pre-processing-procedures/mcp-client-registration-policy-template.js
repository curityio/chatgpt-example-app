...
function result(context) {
  ...
  if (httpMethod === "POST") {
    ...
    // ChatGPT apps seem to not use MCP's scope selection strategy
    var body = request.getParsedBodyAsJson();
    if ((body && body.scope && body.scope.split(" ").indexOf("portfolio") !== -1) ||
        (body && body.client_name && body.client_name === 'ChatGPT')
    ) {
